...
def process(input_csv: str, redis_host: str = 'redis', redis_port: int = 6379):
    ...
    logger.info(
        f'CSV file {input_csv} loaded. Number of rows: {raw_df.count()}')
    logger.info('Casting column "capital_social" to double...')
    df = raw_df.withColumn('capital_social', col(
        'capital_social').cast('double'))

    logger.info('Computing features...')
    features_df = compute_features(df)
    logger.info(f'Features computed. Number of cities: {features_df.count()}')
    write_features_to_redis(features_df, host=redis_host,
                            port=redis_port, logger=logger)
